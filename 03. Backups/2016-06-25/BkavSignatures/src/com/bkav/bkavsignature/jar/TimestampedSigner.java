...
/**
 * This class implements a content signing service. It generates a timestamped
 * signature for a given content according to
 * <a href="http://www.ietf.org/rfc/rfc3161.txt">RFC 3161</a>. The signature
 * along with a trusted timestamp and the signer's certificate are all packaged
 * into a standard PKCS #7 Signed Data message.
 *
 * @author Vincent Ryan
 */
public final class TimestampedSigner extends ContentSigner {
	 ...
	 * timestamp. This method is used when a signature has already been
	 * generated. The signature, a signature timestamp, the signer's certificate
	 * chain, and optionally the content that was signed, are packaged into a
	 * PKCS #7 signed data message.
	 *
	 * @param parameters
	 *            The non-null input parameters.
	 * @param omitContent
	 *            true if the content should be omitted from the signed data
	 *            message. Otherwise the content is included.
	 * @param applyTimestamp
	 *            true if the signature should be timestamped. Otherwise
	 *            timestamping is not performed.
	 * @return A PKCS #7 signed data message including a signature timestamp.
	 * @throws NoSuchAlgorithmException
	 *             The exception is thrown if the signature algorithm is
	 *             unrecognised.
	 * @throws CertificateException
	 *             The exception is thrown if an error occurs while processing
	 *             the signer's certificate or the TSA's certificate.
	 * @throws IOException
	 *             The exception is thrown if an error occurs while generating
	 *             the signature timestamp or while generating the signed data
	 *             message.
	...
	public byte[] generateSignedData(ContentSignerParameters parameters,
		...
		if (!(issuerName instanceof X500Name)) {
			...
			X509CertInfo tbsCert = new X509CertInfo(
					signerCertificateChain[0].getTBSCertificate());
			issuerName = (Principal) tbsCert.get(CertificateIssuerName.NAME
		...
		if (applyTimestamp) {
			...
			PKCS9Attributes unsignedAttrs = new PKCS9Attributes(
			...
			signerInfo = new SignerInfo((X500Name) issuerName, serialNumber,
					digestAlgorithmId, AlgorithmId.get(keyAlgorithm),
					signature);
		}
	 ...
	 * <a href="http://www.ietf.org/rfc/rfc3280.txt">RFC 3280</a>). The
	 * extension's <tt>accessMethod</tt> field should contain the object
	 * identifier defined for timestamping: 1.3.6.1.5.5.7.48.3 and its
	 * <tt>accessLocation</tt> field should contain an HTTP URL.
	 *
	 * @param tsaCertificate
	 *            An X.509 certificate for the TSA.
	...
	public static String getTimestampingUrl(X509Certificate tsaCertificate) {
		...
		try {
			byte[] extensionValue = tsaCertificate
					.getExtensionValue(SUBJECT_INFO_ACCESS_OID);
	...
	/*
	 * Returns a timestamp token from a TSA for the given content. Performs a
	 * basic check on the token to confirm that it has been signed by a
	 * certificate that is permitted to sign timestamps.
	 *
	...
	private byte[] generateTimestampToken(byte[] toBeTimestamped)
		...
		TimestampToken tst = new TimestampToken(
				tsToken.getContentInfo().getData());
