...
public class OOXMLValidator {
	...
	public static void main(String[] args) {
		String signedDoc = "S:/WORK/2015/12-2015/SignedDocument/signed.docx";
		byte[] signedData;
		try {
			signedData = FileUtil.readBytesFromFile(signedDoc);
			System.out.println("---> Signature verify code: "
				+ validate(signedData, CertificateValidator.ONLY_OCSP));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}

	public static int validate(byte[] signedData, int ocspOrCRL) {
		if (signedData == null) {
		...
		try {
			...
			for (PackageDigitalSignature signature : list) {
				...
				int certValid = CertificateValidator.verify(signerCert, null, signingTime, ocspOrCRL);
