...
public class XmlUtil {
	...
	public static Certificate[] getCertificateChain(javax.xml.crypto.dsig.XMLSignature signature) {
	...
	public static Date getSigningTime(XMLSignature signature) {
							...
							if (o2 instanceof DOMSignatureProperty) {
								...
								for (Object o3 : listProp) {
									if (o3 instanceof DOMStructure) {
										...
										if ("DATE".equalsIgnoreCase(node.getNodeName())
												|| "TIME".equalsIgnoreCase(node.getNodeName())
												|| "TIMEZONE".equalsIgnoreCase(node.getNodeName())) {
		...
		Date signTime = new Date(dateTime);
		return signTime;
	}
}